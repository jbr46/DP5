...
REQUIREMENTS AND SETUP - DFT FOR NMR SHIFT CALCULATION
...
1) One should have ((MacroModel or TINKER) and (NWChem or Gaussian)), or CASCADE or SGNN.
...
The qml package is only used when running DP5 calculations. If only doing DP4 calculations, qml is
not needed and the import of DP5 on line 47 of PyDP4.py can be commented out. This may be helpful
if difficulties are encountered in installing qml.
...
REQUIREMENTS AND SETUP - ML FOR NMR SHIFT CALCULATION

All the python files are in the attached archive. They are set up to work
from a centralised location. It is not a requirement, but it is probably
best to add the location to the PATH variable.

The script currently is set up to use SGNN or CASCADE for NMR shift prediction.
This setup has several requirements.

1) One should have CASCADE or SGNN, or both. The DP5 folder can contain settings.cfg, 
where the locations for the installed software packages can be given, so that DP5 can 
launch them and use in the workflow. Examples on how to do this are provided in the included
settings.example - this should be renamed to settings.cfg and any relevant changes made.
...
The nmrglue package is only used when using automatic parsing of raw NMR data. If NMR descriptions are
instead provided, nmrglue is not needed and the imports of process_proton and process_carbon on lines
23 and 24 of NMR.py may be commented out.
...
e M062X single point energy calculations, l SGNN NMR prediction, p CASCADE NMR prediction, b both SGNN and CASCADE NMR prediction, 
s calculate DP4 statistics and w for DP5 statistics. The default workflow is gnms, optimum workflow is gnomes. The letter a can
be given in place of s, in the instance DP5 will analyse and assign the provided NMR spectra but not calculate any probabilities.
